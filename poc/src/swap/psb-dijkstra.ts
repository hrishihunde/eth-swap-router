...
export const PSB_CONFIG = {
  ...
  // Rationale: Most efficient swaps complete in 2-4 hops
  MAX_HOPS: 4,
  
  // Minimum cumulative rate to continue exploring a path
  // Rationale: Prune paths that result in too much slippage
  // NOTE: Many legitimate routes produce very small numeric outputs (e.g. WETH per USDC ~0.0005).
  // Setting this too high will prune valid routes. Use a very small threshold instead.
  MIN_CUMULATIVE_RATE: 0.000001,
  
  // Maximum number of paths to explore before early termination
  // Rationale: Bound computation time for real-time routing
  MAX_PATHS_EXPLORED: 1000,
  
  // Minimum edge rate to consider
  // Rationale: Skip extremely unfavorable swaps
  // NOTE: many rates are <1 (e.g. WETH per USDC ~0.0005). Use a small threshold
  // so we don't accidentally filter valid low-magnitude exchange rates.
  MIN_EDGE_RATE: 0.0001,
  
  // Priority queue pruning: max queue size
  // Rationale: Keep memory usage bounded
  MAX_QUEUE_SIZE: 500,
};

export async function buildGraphPSB(): Promise<Graph> {
  const graph: Graph = {};
  console.log('Building routing graph with PSB optimizations...\n');
  ...
  console.log(`PSB Graph built with ${TOKENS.length} tokens (filtered low-rate edges)\n`);
  return graph;
}

// ============================================================================
// PSB-OPTIMIZED DIJKSTRA IMPLEMENTATION
// ============================================================================
...
export function psbDijkstra(
  ...
  type QueueItem = {
    ...
    path: string[];
    hops: RouteHop[];
    hopCount: number;          // PSB: Track hop count
    cumulativeRate: number;    // PSB: Track cumulative rate for pruning
  };
  ...
  while (queue.length > 0) {
    ...
    if (exploredPaths > PSB_CONFIG.MAX_PATHS_EXPLORED) {
      console.log(`PSB early termination: reached ${PSB_CONFIG.MAX_PATHS_EXPLORED} paths limit\n`);
      break;
    }

    if (current.tokenId === endTokenId) {
      console.log(`PSB Dijkstra explored ${exploredPaths} paths`);
      console.log(`  ├─ Pruned by hop limit: ${prunedByHops}`);
      console.log(`  ├─ Pruned by rate threshold: ${prunedByRate}`);
      console.log(`  └─ Pruned by queue limit: ${prunedByQueueLimit}`);
  ...
  console.log(`PSB Dijkstra explored ${exploredPaths} paths - no route found`);
  console.log(`  Pruned: ${prunedByHops + prunedByRate + prunedByQueueLimit} paths\n`);
  return null;
}

export async function runPSBRouter() {
  console.log('\nPSB-OPTIMIZED DIJKSTRA SWAP ROUTER\n');
  console.log('Optimizations:');
  console.log(`  • Max Hops: ${PSB_CONFIG.MAX_HOPS}`);
  console.log(`  • Min Cumulative Rate: ${PSB_CONFIG.MIN_CUMULATIVE_RATE * 100}%`);
  console.log(`  • Max Paths Explored: ${PSB_CONFIG.MAX_PATHS_EXPLORED}`);
  console.log(`  • Min Edge Rate: ${PSB_CONFIG.MIN_EDGE_RATE * 100}%`);
  console.log(`  • Max Queue Size: ${PSB_CONFIG.MAX_QUEUE_SIZE}\n`);
  ...
  for (const scenario of scenarios) {
    ...
    printRoutingResult(result, `PSB DIJKSTRA: ${scenario.from} -> ${scenario.to}`);
  }
}